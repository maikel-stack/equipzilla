...
- **HeyReach (LinkedIn)** (clave `~/.outbound/heyreach_key`, API `https://api.heyreach.io/api/public`, cabecera `X-API-KEY`): cuenta de LinkedIn de Andrés (id 249406). Endpoints útiles: `/campaign/GetAll`, `/campaign/AddLeadsToCampaignV2`, `/list/GetAll`, `/list/CreateEmptyList`, `/list/AddLeadsToListV2`, `/inbox/GetConversationsV2`, `/stats/GetOverallStats`, `/campaign/Pause|Resume`. Las campañas y secuencias se crean en la interfaz; por API se cargan listas ICP, se meten leads en campañas, se leen respuestas y estadísticas. **Solo demanda** (Maikel 14/09): perfiles que compran máquina (gerentes y jefes de maquinaria de constructoras y excavaciones, responsables de compras y logística, jefes de parque de transportes y grúas); mensaje de venta con 1-2 máquinas concretas del stock, precio cerrado y garantía. Nunca «compramos tu máquina».
