...
def tab_dashboard_simple(deals, camp, frio, ads, seo):
    ...
    f = [["EQUIPZILLA · DEMAND ENGINE", "", "", "",
         ...
         ["GMV cerrado", eur(e["gmv"]), eur(META_GMV), eur(META_GMV - e["gmv"])],
         ["GMV en juego (ofertas abiertas)",
          eur(sum(float(x.get("value") or 0) for x in deals if x.get("status") == "open")),
          "", "%d de %d ofertas aún sin importe" % (sin_importe, e["abiertas"])],
         [],
